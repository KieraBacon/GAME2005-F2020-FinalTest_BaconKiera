...
public class CollisionManager : MonoBehaviour
{
    ...
    public BulletBehaviourAABB[] squareBullets;
    ...
    void Update()
    {
        squareBullets = FindObjectsOfType<BulletBehaviourAABB>();
        ...
        for (int i = 0; i < cubes.Length; i++)
        {
            ...
            for (int j = 0; j < squareBullets.Length; j++)
            {
                CheckAABBs(cubes[i], squareBullets[j]);
            }
        }

        for (int i = 0; i < cubes.Length; i++)
        {
            foreach (Contact c in cubes[i].contacts)
            {
                if (c.cube.GetComponent<RigidBody3D>() != null)
                {
                    if(c.cube.GetComponent<RigidBody3D>().bodyType == BodyType.DYNAMIC)
                    {
                        Reflect(c);
                    }
                }
    ...
    private static void Reflect(Contact c)
    {
        Vector3 p = c.cube.transform.position;
        if (c.face == Vector3.forward)
        {
            p.z += c.penetration * 0.5f;
        }
        else if (c.face == Vector3.back)
        {
            p.z -= c.penetration * 0.5f;
        }
        else if (c.face == Vector3.right)
        {
            p.x += c.penetration * 0.5f;
        }
        else if (c.face == Vector3.left)
        {
            p.x -= c.penetration * 0.5f;
        }
        else if (c.face == Vector3.up)
        {
            p.y += c.penetration * 0.5f;
        }
        else if (c.face == Vector3.down)
        {
            p.y -= c.penetration * 0.5f;
        }
        c.cube.transform.position = p;
    }
